...
# Obtenir le répertoire du script pour les chemins relatifs
...
def parse_events(file_path):
    ...
    with open(file_path, mode='r', encoding='utf-8') as file:
        ...
        for line in lines:
            ...
            if len(row) != 11:
                error_msg = f"Erreur dans la ligne : {row} => trop d'éléments"
                print(error_msg)
            ...
            if not event_id or not problem or not choice_a or not choice_a_long or not choice_b or not choice_b_long or not image_front or not image_back:
                error_msg = f"Erreur : l'événement {row} a des informations manquantes."
                print(error_msg)
                raise ValueError(error_msg)
            
            # Check images too
            _image_front_path = os.path.join(images_dir, image_front + '.png')
            _image_back_path = os.path.join(images_dir, image_back + '.png')
            if not os.path.exists(_image_front_path) or not os.path.exists(_image_back_path):
                error_msg = f'Error: image for {event_id} is missing (front: {_image_front_path}, back: {_image_back_path})'
                print(error_msg)
...
_categories_path = os.path.join(_script_dir, 'categories.json')
categories_data = json.load(open(_categories_path))
...
def _get_impact_from_entry(entry, choice):
    ...
    for d in entry[choice]:
        ...
        if which == 'popularity':
            ...
            error_msg = f'Erreur : {which} is not a valid category'
            print(error_msg)
            raise ValueError(error_msg)
...
if __name__ == '__main__':
    file_path = os.path.join(_script_dir, "events.txt")
    ...
    output_csv_path = os.path.join(_script_dir, 'plouf_game_cards.csv')
    with codecs.open(output_csv_path, mode='w', encoding='utf8') as f:
        buf = data + '\n'.join(new_lines)
        f.write(buf)
